...
public class BPlusTree {
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
      ...
      if (left != this.root || left.scanAll().hasNext()) {
          throw new BPlusTreeException("Cannot bulk load into an empty tree");
      }
    ...
    /* BPlusTreeIterator iterates over entire entries of B+ tree leaf
    * Maintain the following invariants:
    *   - leaf is null iff iter is null
    *   - iter is not null iff iter.hasNext()
    * */
