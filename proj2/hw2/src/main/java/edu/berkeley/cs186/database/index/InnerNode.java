...
/* getLeafNode: given inner node and key - > search for given node */
 public BPlusNode getLeafNode(InnerNode n, DataBox key ) {
   int starternode = 0;

   DataBox currentnode = n.keys.get(starternode);
   while (key.compareTo(currentnode) > 0 || key.compareTo(currentnode) == 0) {
        if (++starternode == n.keys.size()) {
          break;
        }
      currentnode = n.keys.get(starternode);
   }
    return n.getChild(starternode);
 }
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
      ...
      // check if key follows constraints
      // if we can acommodate new key and pointer -> done
      ...
      // if we overflow -> 2d + 1 trees  -> split into 2
      // return pair <c ,left>
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
      ...
      // right node
      InnerNode r = new InnerNode(metadata,rightKeys,rightChildren);
      ...
      return Optional.of(new Pair<>(middleKey, r.getPage().getPageNum()));
  }

  // See BPlusNode.remove.
  @Override
  public void remove(DataBox key) {
    // Inner Node does not require to remove record
    int index = numLessThanEqual(key, keys);
    BPlusNode child = getChild(index);
    child.remove(key);
  }
  ...
  // get child level for the ith node/leaf
