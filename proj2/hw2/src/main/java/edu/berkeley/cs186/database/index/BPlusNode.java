...
abstract class BPlusNode {
  ...
  // NOTE:  gets the leaf where the node resides
  ...
  // NOTE : if pair inserted does not cause an overflow then return empty
  // else fix the leafs and insert
  // distinguish between leaf node and inner node
  // with inner node we do not copy up, only with leaves we split and copy up
